...
const WeekCalendar = defineComponent({
  ...
  setup(props, { slots, attrs, emit, expose }) {
    ...
    let weeklyData: any = { month: '', week: [] };

    const adapter = {
      ...baseAdapter,
      setWeeklyData: (data: any) => {
        weeklyData = data;
      },
    ...
    const renderHeader = () => {
      ...
      const { month, week } = foundation.getWeeklyData(displayValue, dateFnsLocale.value);
    ...
    const renderAllDayEvents = (events: any[]) => {
      ...
      return events.map((event, ind) => {
        ...
        return h('li', { class: `${cssClasses.PREFIX}-event-item ${cssClasses.PREFIX}-event-allday`, key: key || `allDay-${ind}`, style }, [normalizeNode(children)]);
      });
    };
