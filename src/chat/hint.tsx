...
import { IconArrowRight } from '../icons/generated';
...
const Hint = defineComponent({
  ...
  setup(props) {
    return () =>
      h(
        'section',
        { class: cls(`${PREFIX_HINT}s`, { [props.className as string]: !!props.className }), style: props.style },
        (props.value || []).map((item, index) => {
          ...
          return h('div', { class: `${PREFIX_HINT}-item`, key: index, onClick: () => props.onHintClick?.(item) }, [
            h('div', { class: `${PREFIX_HINT}-content` }, item),
            h(IconArrowRight, { class: `${PREFIX_HINT}-icon` }),
          ]);
        })
      );
  },
